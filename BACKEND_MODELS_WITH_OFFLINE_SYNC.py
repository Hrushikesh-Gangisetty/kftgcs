"""
Django Models - Updated for offline sync deduplication support
Copy this to your Django backend: pavaman_gcs_app/models.py

Changes from BACKEND_MODELS_UPDATED.py:
1. MissionEvent: Added client_id field (UUID) for offline-queue dedup
2. MissionEvent: Added CRITICAL to EVENT_STATUS_CHOICES
3. MissionEvent: Added MANUAL_FLIGHT_STARTED, MANUAL_FLIGHT_ENDED to EVENT_TYPES
4. MissionSummary: Added client_id field (UUID) for offline-queue dedup
5. MissionSummary: Added alerts_count field (was in consumer but missing from model)
6. Mission: Added client_id field for mission_status dedup
...
class MissionEvent(models.Model):
    ...
    EVENT_ERROR = "ERROR"
    EVENT_CRITICAL = "CRITICAL"        # ← NEW: Android sends CRITICAL for low battery

    EVENT_STATUS_CHOICES = [
        (EVENT_INFO, "Info"),
        (EVENT_WARNING, "Warning"),
        (EVENT_ERROR, "Error"),
        (EVENT_CRITICAL, "Critical"),   # ← NEW
    ]

    EVENT_TYPES = [
        ("MISSION_STARTED", "Mission Started"),
        ("MANUAL_FLIGHT_STARTED", "Manual Flight Started"),    # ← NEW: from UnifiedFlightTracker
        ...
        ("MANUAL_FLIGHT_ENDED", "Manual Flight Ended"),        # ← NEW: from UnifiedFlightTracker
    ]
    ...
    # UUID stamped by the Android app at enqueue time.
    # Used to reject duplicate deliveries from the offline queue.
    client_id = models.CharField(
        max_length=36,
        null=True,
        blank=True,
        db_index=True,
        help_text="Client-generated UUID for offline queue deduplication"
    )
...
class MissionSummary(models.Model):
    ...
    # ← NEW: Was saved by consumer but missing from model
    alerts_count = models.IntegerField(default=0, help_text="Number of alerts during mission")

    # Project, plot, and crop type from completion dialog
